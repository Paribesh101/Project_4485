...
def extract_names(text):
    patient_match = re.search(r'(Patient|Patient name):\s([A-Z][a-z]+(?:\s[A-Z][a-z]+){1,2})(?=\n|$)', text)
    provider_match = re.search(r'(Provider|Provider name):\sDr\.\s([A-Za-z]+(?:\s[A-Za-z]+)*)(?=,\sMD)', text)
